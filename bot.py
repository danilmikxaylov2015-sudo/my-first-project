...
def safe_birthday_date(year, month, day):
    try:
        ...
        # Для 29 февраля в невисокосный год считаем 28 февраля
...
def decades_keyboard():
    buttons = []
    row = []

    for start in range(MIN_YEAR, MAX_YEAR + 1, 10):
        end = min(start + 9, MAX_YEAR)
        row.append(
            InlineKeyboardButton(
                f"{start}-{end}",
                callback_data=f"decade:{start}"
            )
        )
...
def years_keyboard(start_year):
    buttons = []
    row = []

    end_year = min(start_year + 9, MAX_YEAR)
    ...
    buttons.append([InlineKeyboardButton("⬅️ Назад к годам", callback_data="back_decades")])
    buttons.append([InlineKeyboardButton("❌ Отмена", callback_data="cancel")])
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        "Можно добавить человека, выбрать год рождения от 1900 до 2026, месяц и день.\n\n"
        "Выбери действие:",
...
async def handle_text(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if mode == "waiting_name":
        ...
        await update.message.reply_text(
            f"Имя: {name}\n\n"
            "Теперь выбери год рождения:",
            reply_markup=decades_keyboard()
        )
        return
...
async def button_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("decade:"):
        start_year = int(data.split(":", 1)[1])

        await query.edit_message_text(
            f"Выбери год рождения:",
            reply_markup=years_keyboard(start_year)
        )
        return
