...
def post_json(url, headers, payload, timeout=120):
    ...
    try:
        ...
        raise RuntimeError(f"HTTP {e.code}: {error_text[:1500]}")

    except Exception as e:
        raise RuntimeError(f"Ошибка соединения: {str(e)[:1000]}")
...
def tavily_search_sync(query: str):
    ...
    search_text += "Найденные источники:\n"
    ...
    if search_context:
        messages.append({
            ...
            "content": (
                ...
                "Используй их для ответа. Не выдумывай факты.\n\n"
                f"{search_context}"
            )
        })
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await update.message.reply_text(
        "Привет! Я ИИ-бот.\n\n"
        "Просто напиши вопрос — отвечу обычным ИИ.\n\n"
