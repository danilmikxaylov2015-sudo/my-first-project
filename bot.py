...
def auto_install():
    """Тихая установка библиотек (как в оригинальном bot2.py), которая не крашит Bothost"""
    try:
        import vkbottle
        import aiohttp
    except ImportError:
        print("[AUTO-INSTALL] Устанавливаю необходимые библиотеки (vkbottle, aiohttp)...")
        try:
            subprocess.check_call([sys.executable, "-m", "pip", "install", "vkbottle", "aiohttp"])
        except Exception:
            subprocess.check_call(["python3", "-m", "pip", "install", "vkbottle", "aiohttp"])
