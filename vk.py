...
def install_libs():
    try:
        ...
        subprocess.check_call([sys.executable, "-m", "pip", "install", "vk-api", "requests", "edge-tts", "Pillow"])
...
# ==================== НАСТРОЙКИ ВЛАДЕЛЬЦА ====================
USER_TOKEN = "vk1.a.qDyPIEg8m7e0gHj_2cAQCYNADTjDjLZekYnJebX7KsCu3EVVV4ztg9F6qA_DMIJYIYFXsn4pdKB88y2BOhiJ6UKnWmVfmw6wQ2OUSuvgw6p3GfSPxko19Ovatt2-Xdh3NjsndztXhsyiCUh5bKvkuvOfEzstU_TUO7jASwXBon1uctOFyTzU0gyCQdFOX1kM"
MY_USER_ID = 848213593
TOKEN_FILE = "connected_users.json"
MISTRAL_API_KEY = "WOU07VcpYQ1jREfwMZLogrbTl7m6DMYs"
# =============================================================

# 100% надежная загрузка шрифта на жесткий диск
FONT_FILE = "roboto_black.ttf"
if not os.path.exists(FONT_FILE):
    try:
        print("📥 Скачиваю крутой шрифт для обложки...")
        r = requests.get("https://raw.githubusercontent.com/googlefonts/roboto/main/src/hinted/Roboto-Black.ttf", timeout=15)
        with open(FONT_FILE, "wb") as f:
            f.write(r.content)
        print("✅ Шрифт успешно сохранен!")
    except Exception as e:
        print(f"⚠️ Ошибка скачивания шрифта: {e}")
...
def auto_accept_loop(user_id, token):
    try:
        ...
        print(f"❌ Не удалось запустить автоприем для ID {user_id}: {e}")
        return

    # Если конкретную заявку временно не удалось принять, не долбим её
    # каждый проход и не мешаем обработке остальных заявок.
    failed_until = {}

    print(f"🤝 Автоприем заявок запущен для ID {user_id}")

    while user_id in auto_accept_users:
        try:
            # out=0 явно означает входящие заявки в друзья.
            reqs = vk.friends.getRequests(count=1000, out=0)
            items = reqs.get('items', []) if reqs else []

            if not items:
                time.sleep(10)
                continue

            print(f"📥 ID {user_id}: найдено входящих заявок — {len(items)}")

            for uid in items:
                if user_id not in auto_accept_users:
                    break

                # После неудачи даём конкретной заявке 5 минут отдыха,
                # но продолжаем принимать остальные.
                if failed_until.get(uid, 0) > time.time():
                    continue

                accepted = False
                last_error = None

                # Короткие временные ошибки VK пробуем пережить повтором.
                for attempt in range(3):
                    if user_id not in auto_accept_users:
                        break
                    try:
                        result = vk.friends.add(user_id=uid)
                        print(f"✅ ID {user_id}: заявка от id{uid} обработана, ответ VK: {result}")
                        accepted = True
                        failed_until.pop(uid, None)
                        break
                    except Exception as e:
                        last_error = e
                        print(f"⚠️ ID {user_id}: не удалось принять id{uid}, попытка {attempt + 1}/3: {e}")
                        if attempt < 2:
                            time.sleep(3 + attempt * 2)

                if not accepted:
                    failed_until[uid] = time.time() + 300
                    print(f"⏳ ID {user_id}: id{uid} пропущен на 5 минут. Последняя ошибка: {last_error}")

                # Небольшая пауза между запросами, чтобы не упираться в антифлуд VK.
                time.sleep(1.5)

            # После пачки сразу перепроверяем список, а не ждём 20 секунд.
            time.sleep(3)

        except Exception as e:
            print(f"❌ Ошибка в цикле автоприема для ID {user_id}: {e}")
            time.sleep(10)

    print(f"🛑 Автоприем заявок остановлен для ID {user_id}")

def auto_status_loop(user_id, token):
    try:
        vk_session = vk_api.VkApi(token=token, api_version='5.131')
        vk = vk_session.get_api()
    except: return
    ...
    frames = ["⏳", "⚡", "🔥", "🚀"]
    i = 0
    while user_id in auto_status_users:
        try:
            ...
            status_text = f"{frames[i % len(frames)]} MSK Time: {time_str} | Date: {date_str} 👾"
            vk.status.set(text=status_text)
            i += 1
        except Exception as e:
            print(f"Ошибка автостатуса: {e}")
        time.sleep(60)
        ...
        try:
            ...
            for event in longpoll.listen():
                ...
                if event.type == VkEventType.MESSAGE_NEW:
                    ...
                    # --- АВТО-ФУНКЦИИ (Работают в фоне на чужие сообщения) ---
                    ...
                    if event.from_me and text.startswith("/"):
                        ...
                        admin_cmds = [
                            "/кик", "/спам", "/негатив", "/унегатив", "/клон", "/уклон", 
                            "/реакция", "/стопреакция", "/опубликовать", "/группы", "/игнор", 
                            "/уигнор", "/пригласить", "/дрвчат", "/рассылка", "/отпрчелу", 
                            "/пиар", "/стоппиар", "/гс", "/автозаявка", "/ии", "/обложка", "/статус"
                        ]
                        ...
                        # Исполнение админ-команд
                            ...
                            try:
                                vk.messages.edit(peer_id=peer_id, message_id=message_id, message="🎨 Генерирую дизайн...")
                                
                                user_info = vk.users.get(user_ids=[user_id], fields="photo_max_orig")[0]
                                raw_name = user_nicknames.get(user_id, f"{user_info['first_name']} {user_info['last_name']}")
                                name_eng = cyr_to_eng(raw_name).upper()
                                ...
                                # Загрузка надежного шрифта с диска
                                try:
                                    font_name = ImageFont.truetype(FONT_FILE, 80)
                                    font_time = ImageFont.truetype(FONT_FILE, 140)
                                    font_date = ImageFont.truetype(FONT_FILE, 45)
                                except:
                                    font_name = font_time = font_date = ImageFont.load_default()

                                bg = Image.new("RGBA", (1920, 716), "#09090E")
                                draw = ImageDraw.Draw(bg)
                                draw.ellipse((-400, -400, 800, 800), fill="#7000FF") 
                                draw.ellipse((1200, 200, 2400, 1400), fill="#00E5FF") 
                                bg = bg.filter(ImageFilter.GaussianBlur(150))
                                draw = ImageDraw.Draw(bg)
                                
                                try:
                                    av = Image.open(BytesIO(requests.get(avatar_url).content)).convert("RGBA").resize((300, 300))
                                    mask = Image.new("L", (300, 300), 0)
                                    ImageDraw.Draw(mask).ellipse((0, 0, 300, 300), fill=255)
                                    draw.ellipse((790, 50, 1130, 390), fill="#00E5FF")
                                    draw.ellipse((800, 60, 1120, 380), fill="#09090E")
                                    bg.paste(av, (810, 70), mask)
                                except Exception as e:
                                    print(f"Аватар не загружен: {e}")
                                
                                # Мощный и ровный текст
                                try:
                                    draw.text((960, 460), name_eng, fill="white", font=font_name, anchor="mm")
                                    draw.text((960, 580), time_str, fill="white", font=font_time, anchor="mm")
                                    draw.text((960, 670), date_str, fill="#00E5FF", font=font_date, anchor="mm")
                                except Exception as text_e:
                                    print(f"Ошибка текста: {text_e}")
                                
                                output = BytesIO()
                                bg.save(output, format="PNG")
                                output.seek(0)
                                
                                photo = upload.photo_messages(output)[0]
                                attachment = f"photo{photo['owner_id']}_{photo['id']}"
                                
                                vk.messages.delete(message_ids=message_id, delete_for_all=1)
                                vk.messages.send(
                                    peer_id=peer_id, 
                                    message="✅ Твоя обложка готова!\n\n(ВК блокирует авто-установку на личные профили, скачай картинку и установи её в настройках ручками).", 
                                    attachment=attachment, 
                                    random_id=random.randint(1, 1000000)
                                )
                            except Exception as img_err:
                                try: vk.messages.edit(peer_id=peer_id, message_id=message_id, message=f"❌ Ошибка генерации обложки: {img_err}")
                                except: pass
                            continue
                            ...
                            if len(parts) > 1:
                                ...
                                if action == "вкл":
                                    if user_id not in auto_status_users:
                                        auto_status_users.add(user_id)
                                        threading.Thread(target=auto_status_loop, args=(user_id, token), daemon=True).start()
                                        vk.messages.edit(peer_id=peer_id, message_id=message_id, message="✅ Живой статус ВКЛЮЧЕН. (Время и дата будут обновляться раз в минуту)")
                                    else:
                                        vk.messages.edit(peer_id=peer_id, message_id=message_id, message="⚠️ Авто-статус уже работает.")
                                elif action == "выкл":
                                    if user_id in auto_status_users:
                                        auto_status_users.remove(user_id)
                                        vk.messages.edit(peer_id=peer_id, message_id=message_id, message="🛑 Живой статус ВЫКЛЮЧЕН.")
                                    else:
                                        vk.messages.edit(peer_id=peer_id, message_id=message_id, message="⚠️ Авто-статус не был включен.")
                            else:
                                vk.messages.edit(peer_id=peer_id, message_id=message_id, message="⚠️ Укажите действие! Пример: /статус вкл | выкл")
                            continue
                            ...
                            try:
                                ...
                                if response.status_code == 200:
                                    ...
                                    if len(reply_text) > 4000:
                                        reply_text = reply_text[:3950] + "...\n[Ответ обрезан из-за лимитов ВК]"
                                    vk.messages.edit(peer_id=peer_id, message_id=message_id, message=f"🧠 {reply_text}")
                                else:
                                    error_msg = data.get('error', {}).get('message', str(data))
                                    vk.messages.edit(peer_id=peer_id, message_id=message_id, message=f"❌ Ошибка ИИ: {error_msg}")
                            except Exception as e:
                            ...
                            if user_id in auto_accept_users:
                                ...
                                threading.Thread(target=auto_accept_loop, args=(user_id, token), daemon=True).start()
                                vk.messages.edit(peer_id=peer_id, message_id=message_id, message="✅ Авто-прием заявок ВКЛЮЧЕН. Бот соберет все старые заявки и будет одобрять новые.")
                            continue
                            ...
                            threading.Thread(target=pr_loop, args=(user_id, peer_id, token, pr_text_arg), daemon=True).start()
                        ...
                        # Базовые / Общие команды
                            ...
                            try:
                                help_msg = (
                                    ...
                                    "🛡️ Администратор:\n"
                                    "• /обложка — крутая профильная обложка с датой 🎨\n"
                                    "• /статус вкл/выкл — живой текстовый статус со временем 👾\n"
