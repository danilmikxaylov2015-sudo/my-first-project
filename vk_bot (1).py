...
CEREBRAS_API_KEY = os.getenv("CEREBRAS_API_KEY", "csk-ph2w5j3tthvhrfhd4n6vw4eypkecj58hppf2eef6y5cte3vy").strip()
CEREBRAS_MODEL = os.getenv("CEREBRAS_MODEL", "llama3.1-8b").strip() or "llama3.1-8b"
CEREBRAS_API_URL = os.getenv("CEREBRAS_API_URL", "https://api.cerebras.ai/v1/chat/completions").strip()
MAX_WARNS = 3
...
async def ask_cerebras(prompt, user_context=""):
    ...
    async with aiohttp.ClientSession(timeout=timeout, trust_env=True) as session:
        async with session.post(CEREBRAS_API_URL, headers=headers, json=payload) as response:
            raw_text = await response.text()
            if response.status >= 400:
                raise RuntimeError(f"Cerebras API error {response.status}: {raw_text[:300]}")
            data = json.loads(raw_text)

    choices = data.get("choices") or []
    if not choices:
        raise RuntimeError("Cerebras returned empty choices")
...
async def handle_chat(message: Message):
    ...
    if text == "/start":
        ...
        try:
            answer = await ask_cerebras(prompt, user_context=f"user_id={user_id}, display_name={user_name}")
            await message.answer(
                "🤖 | AI\n"
                "━━━━━━━━━━━━━━━━━━━━\n\n"
                f"{answer}"
            ...
            log.error(f"Cerebras AI error in chat {chat_id} from {user_id}: {e}")
            error_text = str(e)
